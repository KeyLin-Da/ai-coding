...
4. ✅ 生成对应的测试代码
5. ✅ **执行单元测试并生成报告**：运行测试；如有失败，定位失败类/方法并写入异常测试清单，等待用户后续重新执行 Skill 触发重跑；最终将结果输出到 `docs/{需求号}/junit/`
...
8. **测试完成后必须生成报告到 `docs/{需求号}/junit/` 目录**。
...
1. **读取失败记录**：优先读取 `docs/{需求号}/junit/` 下的报告；如果报告不存在，再读取 `target/surefire-reports`。
...
**Q: 单元测试报告保存在哪里？**  
A: 报告会自动生成到 `docs/{需求号}/junit/` 目录下，其中 `{需求号}` 需要从 Git commit 信息或用户输入中提取。如果无法提取需求号，需要向用户询问。
